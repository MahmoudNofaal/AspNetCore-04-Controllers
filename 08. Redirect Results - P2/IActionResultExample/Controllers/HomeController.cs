...
public class HomeController : Controller
{
   ...
   public IActionResult Index()
   {
      ...
      //return RedirectToActionPermanent("Books", "Store", new { id = bookId}); // 301 - Permentely moved

      // we can use local redirect
      // and that restrict the redirect to the another url must be in the same application url
      // but the best practices : RedirectToAction is BETTER

      //return new LocalRedirectResult($"store/books/{bookId}");
      //return LocalRedirect($"store/books/{bookId}"); // 302 - Found
      //return LocalRedirectPermanent($"store/books/{bookId}"); // 301 - Permentely moved

      // we can use Redirect too
      //return Redirect($"store/books/{bookId}"); // 302 - Found
      return RedirectPermanent($"store/books/{bookId}"); // 301 - Permanently Moved
